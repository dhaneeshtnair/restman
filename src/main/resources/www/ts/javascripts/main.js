...
function loadStaticContent(filename,content) {
    //appFileService.readFile(filename, function(content){
        //selectFileName = filename;
        syncStop = true;
        var data= content.replace(/\r\n?/g,"\n");
        typeScriptLS.updateScript(filename, data, false);
        syncStop = false;
    //});
}
...
$(function(){
    ...
    workerOnCreate(function(){//TODO use worker init event
        ...
        ["typescripts/lib.d.ts","typescripts/lib.mc.ts"].forEach(function(libname){
            appFileService.readFile(libname, function(content){
                var params = {
                    data: {
                        name:libname,
